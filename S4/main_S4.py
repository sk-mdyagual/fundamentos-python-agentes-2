...
from pseudo_agente_S4 import AgenteAdmin, PseudoAgente
...
# Bucle de login sin usar break; termina por éxito o por agotar intentos
while intento < MAX_INTENTOS and not tiene_acceso:
    intento += 1
...
if rol == "admin":
    ...
    mi_agente = PseudoAgente("Athena_Invitado")
...
while pseudo_activo:
    if mi_agente.tokens <= 0:
        ...
        mi_agente.registrar_log("sistema", rol, "Agente apagado por falta de tokens.")
        pseudo_activo = False
        continue
  ...
  # --- Comando historial (motor de búsqueda) ---
        ...
        # Usa .split() para separar el comando y sus argumentos
        partes = comando.split()
        if len(partes) == 2 and partes[1] == "all":

            # Muestra todo el historial
            resultado_historial = mi_agente.gestionar_historial("all", rol)
            if isinstance(resultado_historial, list):
                if len(resultado_historial) == 0:
                    print("\n[PseudoAgente] No hay registros en el historial.")
                else:
                    print("\n--- Historial completo ---")
                    for registro in resultado_historial:
            ...
            print(mi_agente.gestionar_historial("clear", rol))
            ...
            print("\n" + gestionar_historial(palabra_busqueda, mi_agente.historial_chat))
            mi_agente.tokens -= 5
    ...
    # --- Comando dado ---
        ...
        resultado_dado = mi_agente.lanzar_dado()
        print(resultado_dado)
        mensaje = f"Se lanzó un dado de 6 caras.'{resultado_dado}'"
