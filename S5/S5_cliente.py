...
def enviar_mensaje_http(remitente: str, destinatario: str, contenido: str) -> dict:
    ...
    print(f"[Cliente] Enviar mensaje - Status: {respuesta.status_code}, Response: {respuesta.json()}")
    return respuesta.json()
...
enviar_mensaje_http("sami", "Atlas", "Reporte de la mision enviado por HTTP.")
print("Mensaje enviado via HTTP.")
# CONCLUSION:
# - requests.get() y requests.post() permiten comunicarse con el servidor
#   desde Python, sin usar el navegador.
...
# CONCLUSION:
# - El circuito completo es: Python (cliente) -> HTTP -> FastAPI (servidor) -> Python (lógica de agentes).
# - Puedes usar el navegador o un script Python para comunicarte con el servidor.
