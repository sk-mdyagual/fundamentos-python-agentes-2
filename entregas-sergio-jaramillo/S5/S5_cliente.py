...
# CONCLUSION: requests.get() y requests.post() hacen exactamente lo mismo que Swagger UI
# pero desde codigo Python. La clave es que el resultado de .json() ya es un diccionario
# de Python listo para usar, no texto crudo. El status_code permite manejar errores de
# forma programatica: 200 es exito, 404 es no encontrado, cualquier otro es inesperado.
...
# CONCLUSION: El circuito completo demuestra que el servidor no sabe ni le importa quien
# hace la peticion: Swagger UI, este script, o una app movil son todos "clientes HTTP".
# El servidor solo lee el JSON que llega, procesa la logica, y retorna JSON.
# Este es el patron de los microservicios y las APIs de los LLMs como Claude o OpenAI:
# tu codigo Python llama a un servidor HTTP externo exactamente igual que este script
# llama al servidor FastAPI local.
