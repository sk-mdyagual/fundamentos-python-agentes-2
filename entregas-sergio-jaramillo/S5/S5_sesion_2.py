...
# CONCLUSION: FastAPI convierte un simple diccionario de Python en una respuesta JSON valida
# sin ninguna configuracion extra. El decorador @app.get("/") "registra" la funcion como
# handler de esa ruta. Swagger UI en /docs se genera automaticamente leyendo los decoradores
# y type hints del codigo — es documentacion viva que siempre esta actualizada.
...
# CONCLUSION: Los path parameters hacen que la URL sea expresiva: /agente/Atlas le dice al
# servidor exactamente que quiero, sin necesidad de un body. HTTPException lanza una respuesta
# de error HTTP estandar — el cliente recibe un JSON con el campo "detail" y el codigo 404.
# FastAPI convierte el raise HTTPException en la respuesta HTTP correcta automaticamente,
# igual a como Python convierte un raise ValueError en un error de Python.
...
# CONCLUSION: Pydantic valida los datos de entrada antes de que lleguen a mi funcion.
# Si energia llega como string "hola", FastAPI devuelve un 422 con un mensaje claro
# explicando que campo fallo y por que — sin que yo escriba una sola linea de validacion.
# POST + BaseModel es el patron estandar para recibir datos estructurados en una API REST.
# GET lee, POST crea: esa separacion de responsabilidades es la base del diseno RESTful.
