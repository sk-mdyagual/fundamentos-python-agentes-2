...
if __name__ == "__main__":
    ...
    # CONCLUSION: Cada vez que ejecuto el script, el diccionario `agente` se crea desde cero.
    # No hay forma de recuperarlo entre ejecuciones porque vive en la RAM del proceso Python.
    # Es como escribir en una pizarra que se borra sola cuando apagas la computadora.
    ...
    # CONCLUSION: SQLite crea un archivo binario `.db` en el disco. No se puede leer con un editor
    # de texto normal porque el formato es binario (no ASCII). Para leerlo se necesita un cliente
    # SQL o la libreria sqlite3 de Python. La clausula IF NOT EXISTS es clave: hace que el script
    # sea idempotente, es decir, puedo ejecutarlo N veces sin duplicar ni romper las tablas.
    ...
    # CONCLUSION: El segundo intento de registrar 'Atlas' devuelve el mensaje de error del
    # except IntegrityError porque `nombre` es PRIMARY KEY — no puede haber duplicados.
    # El uso de `?` como parametro es fundamental: evita que un usuario malicioso inyecte
    # codigo SQL en el input. Por ejemplo, si el nombre fuera "'; DROP TABLE agentes;--"
    # con f-strings eso se ejecutaria, pero con `?` SQLite lo trata como texto literal.
    ...
    # CONCLUSION: Si, el agente Atlas persiste entre ejecuciones porque sus datos estan en el archivo
    # `agentes.db` en el disco, no en la RAM. fetchone() retorna una tupla con los valores de la fila,
    # o None si no existe — por eso la funcion puede retornar `dict | None` con seguridad.
    # La diferencia con el Capitulo 1 es enorme: ahora el agente "recuerda" quien es.
    ...
    # CONCLUSION: Si funciona. La tabla mensajes no tiene ninguna restriccion que impida que
    # remitente == destinatario. Si seria valido o no depende del negocio, no de SQLite.
    # El AUTOINCREMENT del `id` garantiza que cada mensaje tiene un identificador unico
    # sin que yo tenga que calcularlo. El timestamp en formato ISO permite ordenar cronologicamente.
    ...
    # CONCLUSION: WHERE filtra filas segun una condicion, igual que un `if` pero dentro de la base de datos.
    # fetchall() trae todas las filas que coinciden como lista de tuplas. ORDER BY timestamp
    # garantiza que los mensajes aparezcan en orden cronologico sin importar el orden de insercion.
    # La conversion de tuplas a dicts en la funcion hace que el codigo del for sea mucho mas legible.
    ...
    for ag in listar_agentes():
        print(f"  {ag['nombre']} | Rol: {ag['rol']} | Energia: {ag['energia']}")
    ...
    # CONCLUSION: Si, tanto los agentes como los mensajes persisten entre sesiones de Python.
    # Todo queda grabado en `agentes.db`. listar_agentes() hace un SELECT sin WHERE, devolviendo
    # todas las filas de la tabla. Combinar INSERT + SELECT + WHERE construye el patron basico
    # de cualquier base de datos: crear datos, guardarlos, consultarlos y filtrarlos por criterio.
